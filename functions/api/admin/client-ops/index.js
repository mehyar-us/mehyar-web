...
export async function onRequestGet({ request, env }) {
  ...
  const [leadRows, appointmentRows, replyRows, counts] = await Promise.all([
    env.LEADS_DB.prepare(`SELECT id, created_at, updated_at, form_type, status, name, email, phone, company, website, service_interest, budget_range, timeline, message, source, notification_status
      FROM leads ORDER BY created_at DESC LIMIT 150`).all(),
    env.LEADS_DB.prepare(`SELECT id, lead_id, created_at, status, starts_at, ends_at, timezone, name, email, phone, company, service_interest, notes, zoho_event_id, zoho_view_url, client_email_status, owner_email_status, provider_status, provider_error
