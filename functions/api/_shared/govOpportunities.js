...
export async function runGovOpportunityIngest({ env, now = new Date(), fetchImpl = fetch } = {}) {
  ...
  // NAICS filter for SAM.gov — optional. The title-only search already returns
  // decent coverage; adding naics= narrows it dramatically (3 opps → 3 opps) and
  // causes the brief gen to be skipped for already-seen IDs. Pass naics only if
  // explicitly enabled via GOV_USE_NAICS=1.
  const DEFAULT_NAICS = [
    "541511", "541512", "541513", "541519",
    "541611", "541612", "541618", "541690",
    "541990", "511210", "518210", "541715", "541720",
  ];
  const useNaics = env?.GOV_USE_NAICS === "1" || env?.GOV_USE_NAICS === "true";
  const naicsFilter = useNaics
    ? (parseNaics(env?.GOV_NAICS).length ? parseNaics(env?.GOV_NAICS) : DEFAULT_NAICS)
    : [];
