...
const fulfillHooks = {
  ...
  // OpenSeason season packs. Creates the openseason_orders row (idempotent on
  // payment_id's UNIQUE constraint), unifies the access token onto the
  // billing_payments row, then emails the buyer their token-gated pack link
  // immediately (deterministic data — no generation step).
  async openseason({ db, env, waitUntil }, payment) {
    const sendEmail = (e, msg) => sendCloudflareEmail(e, msg);
    await fulfillOpenseason({ db, env, waitUntil, sendEmail }, payment);
  },
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (paymentId) {
        ...
        if (payment) {
          ...
          const ORDER_HOOKS = new Set(["designful","freelanceros","hustlekit","creditfixkit","sprint30","bizbuilder","prepguide","tiktokgrowth","promptpack","truesketch","openseason"]);
          if (!duplicate) {
            await db.prepare(
