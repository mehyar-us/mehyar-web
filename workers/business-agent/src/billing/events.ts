...
function subscriptionWrite(env: BillingEnv, owned: OwnedSubscription, event: StripeEvent, overrides: { paidThrough?: string | null; access?: string; grace?: string | null; invoiceId?: string | null } = {}) {
  ...
  return stmt(env, `INSERT INTO agent_billing_subscriptions (tenant_id,stripe_subscription_id,plan_id,status,paid_through,cancel_at_period_end,updated_at,stripe_customer_id,price_id,billing_interval,catalog_version,activation_order_id,last_event_created,last_invoice_id,grace_expires_at,access_state)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(tenant_id) DO UPDATE SET
    stripe_subscription_id=excluded.stripe_subscription_id,plan_id=excluded.plan_id,status=excluded.status,paid_through=excluded.paid_through,cancel_at_period_end=excluded.cancel_at_period_end,updated_at=excluded.updated_at,stripe_customer_id=excluded.stripe_customer_id,price_id=excluded.price_id,billing_interval=excluded.billing_interval,catalog_version=excluded.catalog_version,activation_order_id=excluded.activation_order_id,last_event_created=MAX(agent_billing_subscriptions.last_event_created,excluded.last_event_created),last_invoice_id=COALESCE(excluded.last_invoice_id,agent_billing_subscriptions.last_invoice_id),grace_expires_at=excluded.grace_expires_at,access_state=excluded.access_state`,
  order.tenant_id, current.id, order.plan_id, current.status, paidThrough, current.cancel_at_period_end ? 1 : 0, new Date().toISOString(), customer, order.price_id, order.billing_interval, CATALOG_VERSION, order.id, event.created, overrides.invoiceId ?? null, overrides.grace === undefined ? existing?.grace_expires_at ?? null : overrides.grace, access);
}
