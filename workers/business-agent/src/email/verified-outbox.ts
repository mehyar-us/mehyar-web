 ...
 * not a sender: supplier-budget gates must be added
...
export class VerifiedInvitationOutbox {
  ...
  constructor(private readonly env:Env,private readonly clock:()=>number=Date.now){this.outbox=new InvitationEmailOutbox(env,clock);this.allowance=new PlatformEmailAllowance(env,clock);}
  ...
  async claim(tenantId:string,id:string):Promise<EmailClaim|null>{
    ...
    if(!claim){await this.allowance.reconcile(tenantId,id);return null;}
    try{
  ...
  async settle(claim:Pick<EmailClaim,'id'|'tenantId'|'token'>,result:SendResult){const saved=await this.outbox.settle(claim,result);await this.allowance.reconcile(claim.tenantId,claim.id);return saved;}
  async reconcileAuthority(tenantId:string,id:string){const changed=await this.outbox.reconcileAuthority(tenantId,id);await this.allowance.reconcile(tenantId,id);return changed;}
}
