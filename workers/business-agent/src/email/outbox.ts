...
export function invitationEmailAuthoritySql(allowPaused:boolean){return `EXISTS(SELECT 1 FROM agent_team_invitations i JOIN agent_tenants t ON t.id=i.tenant_id JOIN agent_memberships m ON m.tenant_id=i.tenant_id AND m.user_id=i.invited_by
 ...
 * Claims and dispatch checks enforce recorded recipient suppression. A future
 * dispatcher must also enforce sender/account readiness and quotas. */
export class InvitationEmailOutbox {
  ...
  async claim(tenantId:string,id:string,routeRef:string):Promise<EmailClaim|null>{
    ...
    const changed=await this.env.AGENT_DB.prepare(`UPDATE agent_platform_email_outbox SET state='sending',attempts=attempts+1,first_attempt_at=COALESCE(first_attempt_at,?),lease_token=?,lease_expires_at=?
      ...
      AND ${invitationEmailAuthoritySql(false)}`)
  ...
  async reconcileAuthority(tenantId:string,id:string){
    ...
    const changed=await this.env.AGENT_DB.prepare(`UPDATE agent_platform_email_outbox
      ...
      WHERE tenant_id=? AND id=? AND state IN ('prepared','retry','sending') AND (lease_expires_at IS NULL OR lease_expires_at<=?) AND NOT (${invitationEmailAuthoritySql(true)})`)
  ...
  async mayDispatch(claim:EmailClaim,routeRef:string){
    ...
    const row=await this.env.AGENT_DB.prepare(`SELECT payload_hash,idempotency_key,first_attempt_at FROM agent_platform_email_outbox
      WHERE tenant_id=? AND id=? AND route_ref=? AND state='sending' AND lease_token=? AND lease_expires_at>? AND first_attempt_at>? AND first_attempt_at<=? AND ${invitationEmailAuthoritySql(false)}`)
