  ...
  it('resumes paid section analyses without exposing them as whole-message results',async()=>{
    ...
    await runInDurableObject(stub,async(instance:BusinessAgent,ctx)=>{
      ...
      try{
        ...
        const offer=unwrap(await instance.reviewMailboxSections(f.actor,f.streamId,'long',claim.token));
        expect(offer).toMatchObject({textCredits:2,remainingSections:[0,1],scope:'analyze_remaining_sections',includesAggregation:false,authorizesExternalActions:false});
        expect(unwrap(await instance.reviewMailboxSections(f.actor,f.streamId,'long',claim.token))).toEqual(offer);
        expect(unwrap(await instance.usage(f.actor)).textCredits).toMatchObject({used:0,reserved:0});expect(calls).toBe(0);
        expect((await instance.confirmMailboxSection({...f.actor,userId:crypto.randomUUID()},offer.offerId,1)).ok).toBe(false);
        const originalTerms=ctx.storage.sql.exec<{terms:string}>('SELECT terms FROM mailbox_section_offers WHERE id=?',offer.offerId).one().terms;
        ctx.storage.sql.exec('UPDATE mailbox_section_offers SET terms=? WHERE id=?',JSON.stringify({...JSON.parse(originalTerms),period:'changed'}),offer.offerId);
        expect(await instance.confirmMailboxSection(f.actor,offer.offerId,1)).toMatchObject({ok:false,error:{code:'section_offer_changed'}});expect(calls).toBe(0);
        ctx.storage.sql.exec('UPDATE mailbox_section_offers SET terms=? WHERE id=?',originalTerms,offer.offerId);
        const section=unwrap(await instance.confirmMailboxSection(f.actor,offer.offerId,1));
        ...
        await e.AGENT_DB.prepare('UPDATE agent_mailbox_messages SET needs_reconciliation=0 WHERE stream_id=?').bind(f.streamId).run();
        const remaining=unwrap(await instance.reviewMailboxSections(f.actor,f.streamId,'long',claim.token));
        expect(remaining).toMatchObject({textCredits:1,remainingSections:[0]});
        expect(await instance.confirmMailboxSection(f.actor,remaining.offerId,1)).toMatchObject({ok:false,error:{code:'section_offer_changed'}});expect(calls).toBe(1);
        unwrap(await instance.confirmMailboxSection(f.actor,remaining.offerId,0));expect(calls).toBe(2);
        expect(await instance.reviewMailboxSections(f.actor,f.streamId,'long',claim.token)).toMatchObject({ok:false,error:{code:'triage_sections_complete'}});
        expect(unwrap(await instance.usage(f.actor)).textCredits).toMatchObject({used:2,reserved:0});
        ...
        expect(await instance.confirmMailboxSection(f.actor,offer.offerId,0)).toMatchObject({ok:false,error:{code:'section_offer_changed'}});expect(calls).toBe(2);
        ctx.storage.sql.exec('UPDATE mailbox_section_offers SET expires=? WHERE id=?',Date.now()-1,offer.offerId);
        expect(await instance.confirmMailboxSection(f.actor,offer.offerId,0)).toMatchObject({ok:false,error:{code:'section_offer_expired'}});expect(calls).toBe(2);
        (instance as any).env.MAILBOX_EXTENDED_TRIAGE_ENABLED='false';
