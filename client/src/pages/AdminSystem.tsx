            ...
            <div className="overflow-auto max-h-[60vh] space-y-1 text-xs">
              {(audit.data?.items || []).map((e: any) => (
                <div key={e.id} className="flex gap-2 items-start border-l-2 border-zinc-200 pl-2 py-1 hover:bg-zinc-50 dark:bg-zinc-800/50">
                  ...
                  <span className="font-mono text-xs text-zinc-500 dark:text-zinc-400">{(e.prospect_id || e.sam_id || "").slice(0, 8)}</span>
                  <span className="text-zinc-500 dark:text-zinc-400 flex-1 truncate">{String(e.summary || e.payload || "").slice(0, 140)}</span>
                </div>
              ))}
  ...
  return (
    <div id="backup">
      <Card><CardContent className="p-5">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-3">Download a JSON dump of your prospects, opportunities, drafts, replies, and audit events. Up to 5,000 rows per table.</p>
