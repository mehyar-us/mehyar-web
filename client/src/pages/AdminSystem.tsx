    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 80px)" }}>
        ...
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key as any)}
            className={`px-3 py-2 text-sm border-b-2 -mb-px transition ${
              tab === t.key ? "border-emerald-500 text-emerald-700 dark:text-emerald-400 font-medium" : "border-transparent text-zinc-600 hover:text-zinc-900 dark:text-zinc-100"
            }`}>
            {t.label} <span className="text-xs $1 ml-1">{t.desc}</span>
          </button>
        ))}
      </div>
      ...
      {tab === "audit" && (
        <Card><CardContent className="p-4">
          <div className="flex gap-2 mb-3">
            ...
            <span className="text-xs $1 self-center ml-auto">{audit.data?.items?.length || 0} events</span>
          </div>
          {audit.isLoading ? <div className="py-10 text-center text-sm $1"><Loader2 className="inline w-4 h-4 animate-spin mr-2" />Loading…</div> : (
            <div className="overflow-auto max-h-[60vh] space-y-1 text-xs">
              {(audit.data?.items || []).map((e: any) => (
                <div key={e.id} className="flex gap-2 items-start border-l-2 border-zinc-200 pl-2 py-1 hover:bg-zinc-50 dark:bg-zinc-800/50">
                  <span className="font-mono $1 w-32 shrink-0">{new Date(e.created_at).toLocaleString()}</span>
                  <Badge className="text-[10px]">{e.kind || "any"}</Badge>
                  <Badge className="text-[10px] bg-violet-100 text-violet-800 dark:text-violet-300">{e.event_type}</Badge>
                  <span className="font-mono text-xs $1">{(e.prospect_id || e.sam_id || "").slice(0, 8)}</span>
                  <span className="$1 flex-1 truncate">{String(e.summary || e.payload || "").slice(0, 140)}</span>
                </div>
              ))}
      ...
      {tab === "cron" && (
        <Card><CardContent className="p-4">
          ...
          {lastCronResult && (
            <div className={`text-xs px-3 py-2 rounded mb-3 ${lastCronResult.ok ? "bg-emerald-50 border border-emerald-200 text-emerald-800" : "bg-red-50 dark:bg-red-950/30 border border-red-200 text-red-800"}`}>
              ...
              {(cron.data?.items || []).map((r: any) => {
                ...
                return (
                  <tr key={r.id} className="border-b hover:bg-zinc-50 dark:bg-zinc-800/50">
                    ...
                    <td className="p-2"><Badge className={p.ok !== false ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800" : "bg-red-100 dark:bg-red-900/40 text-red-800"}>{p.ok !== false ? "ok" : "fail"}</Badge></td>
                    <td className="p-2 $1">{r.name || "—"}</td>
                    ...
                    <td className="p-2 text-red-700 dark:text-red-400 text-xs">{p.errors ? JSON.stringify(p.errors).slice(0, 60) : ""}</td>
      ...
      {tab === "health" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <Card><CardContent className="p-4">
            <div className="text-xs uppercase $1 mb-2 flex items-center gap-1"><Database className="w-3 h-3" /> D1 database</div>
            {health.isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : (
              <>
                ...
                <div className="text-xs $1 mt-1">{health.data?.db?.tables || "?"} tables · {health.data?.db?.rows_total || "?"} rows</div>
              </>
            )}
          </CardContent></Card>
          <Card><CardContent className="p-4">
            <div className="text-xs uppercase $1 mb-2 flex items-center gap-1"><Sparkles className="w-3 h-3" /> LLM provider</div>
            {health.isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : (
              <>
                <div className="text-2xl font-bold">{health.data?.llm?.provider || "cloudflare"}</div>
                <div className="text-xs $1 mt-1">{health.data?.llm?.model || "?"}</div>
                <Badge className={health.data?.llm?.reachable ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 mt-1" : "bg-amber-100 text-amber-800 dark:text-amber-300 mt-1"}>
          ...
          <Card><CardContent className="p-4">
            <div className="text-xs uppercase $1 mb-2 flex items-center gap-1"><Server className="w-3 h-3" /> Last 24h</div>
            <div className="text-2xl font-bold">{health.data?.errors_24h || 0}</div>
            <div className="text-xs $1 mt-1">{health.data?.requests_24h || 0} API calls · {health.data?.llm_calls_24h || 0} LLM</div>
          </CardContent></Card>
        </div>
      )}
  ...
  return (
    <div id="backup">
      <Card><CardContent className="p-5">
        ...
        <p className="text-sm $1 mb-3">Download a JSON dump of your prospects, opportunities, drafts, replies, and audit events. Up to 5,000 rows per table.</p>
...
function SettingsPanel({ token }: { token: string }) {
  ...
  return (
    <Card><CardContent className="p-5">
      ...
      {!loaded && <div className="text-xs $1 mb-2"><Loader2 className="inline w-3 h-3 animate-spin mr-1" />Loading…</div>}
      <div className="space-y-3 text-sm max-w-xl">
        <div>
          <label className="text-xs $1">Default LLM model</label>
          <Input value={model} onChange={(e) => setModel(e.target.value)} className="text-sm" />
          <p className="text-xs $1 mt-1">Cloudflare Workers AI · e.g. <code>@cf/meta/llama-3.2-3b-instruct</code>, <code>@cf/mistral/mistral-7b-instruct-v0.1</code></p>
        </div>
        <div>
          <label className="text-xs $1">From address (outbound emails)</label>
        ...
        {savedAt && <p className="text-xs text-emerald-700 dark:text-emerald-400">✓ Saved at {savedAt}</p>}
        {error && <p className="text-xs text-red-700 dark:text-red-400">⚠ {error}</p>}
        <p className="text-xs $1 pt-2">Settings take effect on the next scheduled cron run.</p>
      </div>
    </CardContent></Card>
  );
}
