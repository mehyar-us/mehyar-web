...
export default function ProposalPublic() {
  ...
  if (state.loading) return <div className="grid min-h-[70vh] place-items-center"><Sparkles className="h-8 w-8 animate-pulse text-cyan-600" /></div>;
  if (state.error) return <div className="mx-auto max-w-xl px-4 py-28 text-center"><h1 className="text-3xl font-bold">This proposal is not available</h1><p className="mt-3 text-muted-foreground">It may be private or still being prepared.</p><Button className="mt-6" asChild><Link href="/contact">Contact MehyarSoft</Link></Button></div>;
...
function Proposal({ proposal }: any) {
  ...
  return <div className="bg-[#f7f7f2] text-zinc-950 dark:bg-zinc-950 dark:text-white">
    <section className="relative overflow-hidden border-b border-black/10">
      {proposal.hero_url ? <img src={proposal.hero_url} alt={`${content.business?.name || "Business"} growth proposal`} className="absolute inset-0 h-full w-full object-cover" /> : null}<div className="absolute inset-0 bg-gradient-to-r from-zinc-950 via-zinc-950/90 to-zinc-950/35" />
      <div className="relative mx-auto flex min-h-[72vh] max-w-7xl flex-col justify-end px-4 py-16 text-white sm:px-6 md:py-24"><Badge className="mb-5 w-fit bg-cyan-300 text-zinc-950">Prepared for {content.business?.name}</Badge><p className="text-sm font-bold uppercase tracking-[0.2em] text-cyan-200">{content.hero?.eyebrow}</p><h1 className="mt-4 max-w-4xl text-4xl font-black leading-[0.96] tracking-[-0.05em] sm:text-6xl lg:text-7xl">{content.hero?.headline}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-200 sm:text-xl">{content.hero?.subheadline}</p><div className="mt-8 flex flex-wrap gap-3"><Button size="lg" className="bg-cyan-300 text-zinc-950 hover:bg-cyan-200" asChild><Link href="/booking">{content.hero?.primary_cta || "Book a call"}<ArrowRight className="ml-2 h-4 w-4" /></Link></Button><Button size="lg" variant="outline" className="border-white/30 bg-white/10 text-white hover:bg-white/20" asChild><a href={proposal.source_url} target="_blank" rel="noreferrer">Current website <ExternalLink className="ml-2 h-4 w-4" /></a></Button></div></div>
    </section>

    <main className="mx-auto max-w-7xl space-y-16 px-4 py-14 sm:px-6 md:space-y-24 md:py-24">
      ...
      <section className="rounded-[2rem] bg-zinc-950 px-5 py-10 text-white sm:p-10"><div className="grid gap-9 lg:grid-cols-[0.8fr_1.2fr]"><div><Bot className="h-9 w-9 text-cyan-300" /><p className="mt-5 text-xs font-bold uppercase tracking-[0.2em] text-cyan-300">Agentic operations</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">{content.agentic?.headline}</h2><p className="mt-5 leading-7 text-zinc-300">{content.agentic?.summary}</p><div className="mt-5 flex flex-wrap gap-2">{content.agentic?.mobile_channels?.map((channel: string) => <Badge key={channel} className="bg-white/10 text-white">{channel}</Badge>)}</div></div><div className="grid gap-3 sm:grid-cols-2">{content.agentic?.use_cases?.map((item: any, index: number) => <div key={`${item.title}-${index}`} className="rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex items-center gap-2"><MessageSquareText className="h-4 w-4 text-cyan-300" /><h3 className="font-bold">{item.title}</h3></div><p className="mt-3 rounded-lg bg-black/30 p-2 text-xs text-cyan-100">“{item.command}”</p><p className="mt-3 text-sm leading-6 text-zinc-300">{item.action}</p><p className="mt-2 text-xs text-emerald-300">Business value: {item.business_value}</p>{item.approval_required ? <p className="mt-2 flex gap-1 text-[11px] text-amber-200"><ShieldCheck className="h-3 w-3 shrink-0" />You approve before external action.</p> : null}</div>)}</div></div></section>

      <section className="grid gap-6 lg:grid-cols-2"><Card className="bg-gradient-to-br from-fuchsia-50 to-orange-50 dark:from-fuchsia-950/30 dark:to-orange-950/30"><CardContent className="p-6 sm:p-8"><Sparkles className="h-7 w-7 text-fuchsia-600" /><h2 className="mt-4 text-3xl font-black">{content.social?.headline}</h2><p className="mt-3 leading-7">{content.social?.summary}</p><List items={content.social?.monthly_outputs} /></CardContent></Card><Card><CardContent className="p-6 sm:p-8"><Clock3 className="h-7 w-7 text-cyan-600" /><h2 className="mt-4 text-3xl font-black">A practical launch plan</h2><div className="mt-5 space-y-4">{content.roadmap?.map((step: any) => <div key={step.phase} className="border-l-2 border-cyan-400 pl-4"><p className="text-xs font-bold uppercase tracking-wider text-cyan-700">{step.timing}</p><h3 className="font-bold">{step.phase}</h3><p className="mt-1 text-sm text-muted-foreground">{step.outcome}</p></div>)}</div></CardContent></Card></section>
      ...
      <section className="rounded-[2rem] bg-cyan-300 p-7 text-zinc-950 sm:p-12"><h2 className="max-w-3xl text-3xl font-black tracking-tight sm:text-5xl">{content.closing?.headline}</h2><p className="mt-4 max-w-2xl text-lg leading-8">{content.closing?.body}</p><Button size="lg" className="mt-7 bg-zinc-950 text-white" asChild><Link href="/booking">{content.closing?.cta || "Book a call"}<ArrowRight className="ml-2 h-4 w-4" /></Link></Button></section>
...
function SectionTitle({ eyebrow, title, body }: any) { return <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-700">{eyebrow}</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">{title}</h2>{body ? <p className="mt-4 max-w-2xl text-base leading-7 text-muted-foreground">{body}</p> : null}</div>; }
