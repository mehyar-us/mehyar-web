...
function ClientWorkspace({ token }: { token: string }) {
  ...
  return <div className="mx-auto max-w-7xl p-3 pb-[calc(88px+env(safe-area-inset-bottom))] sm:p-5 md:p-6">
    ...
    <header className="mb-4 flex flex-wrap items-start justify-between gap-3">
      ...
      <Badge className={zoho?.connected ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-900"}>{zoho?.connected ? "Calendar connected" : "Calendar needs access"}</Badge>
    </header>
    {!zoho?.connected && !loading ? <Card className="mb-4 border-amber-200 bg-amber-50"><CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><strong>Reconnect business calendar</strong><p className="mt-1 text-sm text-muted-foreground">Required for live availability and event creation.</p></div><Button onClick={connectZoho}>Connect calendar</Button></CardContent></Card> : null}
