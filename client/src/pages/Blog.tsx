        ...
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_0.75fr] lg:items-end">
          ...
          <div className="rounded-[1.5rem] border border-border bg-card/88 p-4 shadow-[0_1px_2px_rgba(10,20,24,0.06)]">
            ...
            <Input
              id="blog-search"
              type="search"
              placeholder="Search by problem or tool..."
              value={searchTerm}
            ...
            {filteredPosts.length > 0 ? (
              <div className="space-y-5">
                {filteredPosts.map((post, index) => (
                  <Card
                    key={post.id}
                    className="border-border bg-card shadow-[0_1px_2px_rgba(10,20,24,0.06)] transition hover:border-brand-700/35"
                  >
                    <CardContent className="grid gap-5 p-5 md:grid-cols-[180px_1fr] md:p-6">
                      <img
                        src={post.image}
                        alt=""
                        className="h-48 w-full rounded-2xl object-cover md:h-full md:min-h-[180px]"
                        loading="lazy"
                      />
                      <div>
                        ...
                        <Link
                          href={`/blog/${post.slug}`}
                          className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-brand-800 hover:text-brand-700 dark:text-brand-100"
                        >
                          Read article{" "}
                        ...
                        {index === 1 ? (
                          <div className="mt-5 rounded-2xl border border-brand-700/20 bg-secondary/70 p-4 dark:border-white/10 dark:bg-white dark:bg-zinc-900/[0.04]">
                            <p className="text-sm font-semibold text-foreground">
                              Want the checklist before the audit?
                            </p>
                            <Link
                              href="/newsletter?utm_campaign=blog_inline_cta"
                              className="mt-2 inline-flex text-sm font-semibold text-brand-800 dark:text-brand-100"
                            >
                              Get the free AI automation checklist →
                            </Link>
                          </div>
                        ) : null}
                      </div>
          ...
          <aside className="space-y-5">
                ...
                <div className="mt-5 space-y-4">
                  {sortedPosts.slice(0, 4).map((post) => (
                    <Link
                      key={post.id}
                      href={`/blog/${post.slug}`}
                      className="block rounded-xl border border-border bg-background/70 p-3 text-sm font-semibold leading-5 text-foreground hover:text-brand-800 dark:hover:text-brand-100"
                    >
                      {post.title}
            ...
            <Card className="border-brand-700/20 bg-secondary/80 shadow-[0_1px_2px_rgba(10,20,24,0.06)] dark:border-white/10 dark:bg-white dark:bg-zinc-900/[0.04]">
              <CardContent className="p-6">
                ...
                <p className="mt-3 leading-7 text-muted-foreground">
                  Turn one article topic into an audit, cleanup, automation, or
