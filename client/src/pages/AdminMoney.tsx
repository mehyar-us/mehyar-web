...
function MoneyView({ token }: { token: string }) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 80px)" }}>
      ...
      {q.isLoading && (
        <div className="text-center py-20 text-zinc-500 dark:text-zinc-400 dark:text-zinc-400"><Loader2 className="inline w-6 h-6 animate-spin mr-2" />Loading pipeline…</div>
      )}
      ...
      {cells.map((c) => (
        <Card key={c.label}>
          <CardContent className="p-3">
            <div className="text-[11px] uppercase tracking-wide text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 leading-tight">{c.label}</div>
          ...
          {data.map((s: any) => {
            ...
            return (
              <div key={s.stage} className="flex items-center gap-2 text-sm">
                <span className="w-24 capitalize text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">{s.stage}</span>
                <div className="flex-1 h-7 bg-zinc-100 dark:bg-zinc-800 rounded overflow-hidden relative">
                  ...
                  <div className="absolute inset-0 flex items-center px-2 text-xs font-medium text-zinc-900 dark:text-zinc-100">
                    ...
                    <span className="ml-auto pr-2 text-xs text-zinc-700 dark:text-zinc-300 dark:text-zinc-300">avg ${Math.round(Number(s.avg_value_usd) || 0).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            );
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        {!deals?.length ? <div className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">No open deals yet.</div> : (
          <div className="space-y-1">
            {deals.slice(0, 12).map((d: any) => (
              ...
              >
                ...
                {d.subtitle && <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mt-0.5 truncate">{d.subtitle}</div>}
              </div>
            ))}
          </div>
        )}
        <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mt-2">💡 Double-click any deal to mark Won (auto-drafts a case study + post-mortem).</div>
      </CardContent>
    </Card>
  );
}
        ...
        <ul className="space-y-1.5 text-sm">
          {deals.map((d: any) => (
            <li key={d.id} className="rounded border border-zinc-200 dark:border-zinc-700 p-2">
              ...
              {d.decision_at && <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mt-0.5">{new Date(d.decision_at).toLocaleDateString()}</div>}
            </li>
          ))}
        </ul>
      </CardContent>
        ...
        {open && (
          <div className="space-y-3 mt-3 pt-3 border-t">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <div>
                <label className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Client name *</label>
                <Input value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Acme Corp" />
              </div>
              <div>
                <label className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Client email</label>
                <Input type="email" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} placeholder="billing@acme.com" />
              </div>
              <div className="md:col-span-2">
                <label className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Address (optional)</label>
                <Input value={clientAddress} onChange={(e) => setClientAddress(e.target.value)} placeholder="123 Main St, City" />
              </div>
              <div>
                <label className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Payment due (days)</label>
                <Input type="number" value={dueDays} onChange={(e) => setDueDays(Number(e.target.value))} />
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Line items</label>
              ...
              <div className="text-right mt-2 text-sm">
                <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Subtotal:</span>{" "}
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mb-3">Click any row to copy a ready-to-paste quote block.</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {SAMPLES.map((s, i) => (
            <button
              key={i}
            ...
            >
              ...
              <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mt-0.5">{s.desc}</div>
  ...
  return (
    <Card className="mt-6">
      <CardContent className="p-4">
        ...
        {data.length === 0 ? (
          <div className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Mark a deal Won and the AI will draft a public case study for SEO.</div>
        ) : (
            ...
            {data.map((cs: any) => (
              <li key={cs.id} className="border rounded p-2 flex items-start gap-2">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">/{cs.slug} · {cs.published ? "🟢 published" : "🟡 draft"}</div>
                </div>
  ...
  return (
    <Card className="mt-6">
      <CardContent className="p-4">
        ...
        {drafts.isLoading && <div className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 py-6 text-center"><Loader2 className="inline w-4 h-4 animate-spin mr-1" />Loading queue…</div>}
        {drafts.error && <div className="text-sm text-red-700">⚠ {String((drafts.error as Error)?.message || drafts.error)}</div>}

        {drafts.data && (
          <>
            <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mb-3">
            ...
            {Array.isArray(drafts.data.items) && drafts.data.items.length > 0 ? (
                ...
                {drafts.data.items.slice(0, 20).map((it: any, i: number) => (
                  <div key={`${it.prospect_id}-${it.step_order}-${i}`} className="border border-zinc-200 rounded-lg p-2.5 bg-white dark:bg-zinc-900 flex items-center gap-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 truncate">
              ...
              <div className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 py-4 text-center">
            ...
            <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800">
              <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 mb-2">
...
function RecentDraftsList({ token, onOpen }: { token: string; onOpen: (id: string) => void }) {
  ...
  if (drafts.isLoading) return <div className="text-xs text-zinc-400 dark:text-zinc-400 py-2">Loading drafts…</div>;
  if (!drafts.data || drafts.data.length === 0) {
    return <div className="text-xs text-zinc-400 dark:text-zinc-400 py-2">No drafts yet — click a pricing tier in CRM to generate one.</div>;
  }
      ...
      {drafts.data.map((d: any) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[10px] text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 font-mono">{d.id.slice(0, 8)} · {d.status} · {new Date(d.created_at).toLocaleDateString()}</div>
          </div>
      ...
      <>
        ...
        <aside className="fixed z-50 bg-white dark:bg-zinc-900 shadow-2xl flex flex-col
          ...
          <div className="sticky top-0 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 z-10 px-4 py-3 md:px-5 flex items-center gap-3">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-zinc-500 dark:text-zinc-400 dark:text-zinc-400 truncate">
                  {prospect?.business_name || sam?.title || "—"} · {draft?.status || "—"}
                </div>
              </div>
            </div>
            <button onClick={onClose} aria-label="Close draft" className="rounded-full p-2 hover:bg-zinc-100 text-zinc-700 dark:text-zinc-300 dark:text-zinc-300 min-h-[44px] min-w-[44px] flex items-center justify-center shrink-0">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain p-4 md:p-5 bg-white text-zinc-900 dark:text-zinc-100" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 80px)" }}>
            ...
            {q.data && !draft && <div className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Draft not found: <code className="text-xs bg-zinc-100 dark:bg-zinc-800 px-1 py-0.5 rounded">{draftId}</code></div>}
              ...
              <div className="space-y-4">
                ...
                <Card><CardContent className="p-4">
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                    <div><span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Source:</span> <strong>{data.kind === "sam" ? "🏛 SAM" : "🧲 Prospect"}</strong></div>
                    <div><span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Model:</span> <code className="text-[11px] bg-zinc-100 dark:bg-zinc-800 px-1 py-0.5 rounded">{draft.model || "?"}</code></div>
                    <div><span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Created:</span> {new Date(draft.created_at).toLocaleString()}</div>
                    <div className="col-span-2 md:col-span-3">
                      <span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Subject:</span>{" "}
                      <strong className="break-words">{draft.subject}</strong>
                    </div>
                    {send && (
                      <>
                        <div><span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">To:</span> <strong>{send.to_email}</strong></div>
                        <div><span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Status:</span> <Badge className="bg-zinc-100 text-zinc-800 dark:text-zinc-200">{send.status}</Badge></div>
                        <div><span className="text-zinc-500 dark:text-zinc-400 dark:text-zinc-400">Scheduled:</span> {send.scheduled_for || "—"}</div>
                      </>
                    )}
                  </div>
                </CardContent></Card>
