...
function MoneyView({ token }: { token: string }) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 80px)" }}>
      ...
      {q.isError && (
        <Card><CardContent className="py-8 text-sm text-red-700 dark:text-red-400">⚠ {String((q.error as Error)?.message || q.error)}</CardContent></Card>
      )}
...
function ForecastStrip({ kpis }: any) {
  const cells = [
    { label: "💎 Pipeline $",           val: kpis.pipeline_value,      tone: "text-cyan-700 dark:text-cyan-400",     prefix: "$" },
    { label: "🔥 Weighted forecast",     val: kpis.weighted_forecast,   tone: "text-orange-700 dark:text-orange-400",   prefix: "$" },
    { label: "🏆 Won (last 30d)",        val: kpis.won_value_30d,       tone: "text-emerald-700 dark:text-emerald-400",  prefix: "$" },
    { label: "📈 Win rate (last 30d)",   val: kpis.win_rate_30d,        tone: "text-emerald-700 dark:text-emerald-400",  suffix: "%" },
    { label: "📊 Avg deal size",         val: kpis.avg_deal_size,       tone: "text-zinc-800 dark:text-zinc-200",     prefix: "$" },
    { label: "🚀 Activity → Win",       val: kpis.activity_to_win,     tone: "text-violet-700 dark:text-violet-400",   suffix: "%" },
    { label: "🧠 AI suggestions today", val: kpis.ai_suggestions_today, tone: "text-blue-700 dark:text-blue-400" },
    { label: "🩸 Outbound failures 24h", val: kpis.outreach_failures_24h, tone: "text-red-700 dark:text-red-400" },
  ];
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {SAMPLES.map((s, i) => (
            <button
              ...
              className="text-left rounded border border-zinc-200 dark:border-zinc-700 hover:border-violet-400 hover:bg-violet-50 dark:bg-violet-950/40/30 transition p-2"
            >
