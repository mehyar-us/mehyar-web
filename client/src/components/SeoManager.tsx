...
const offerCatalog = {
  ...
  itemListElement: services.map((service) => {
    const range = service.features.find((feature) =>
      feature.toLowerCase().includes("typical range"),
    );
...
const industryOfferCatalog = {
  "@type": "OfferCatalog",
  name: "MehyarSoft industry packages",
  itemListElement: industryOffers.flatMap((industry) =>
    industry.packages.map((offer) => ({
      ...
      priceSpecification: {
        "@type": "PriceSpecification",
        priceCurrency: "USD",
        description: `${offer.price} ${offer.cadence}`,
      },
      itemOffered: {
        "@type": "Service",
        name: offer.name,
...
const staticMeta: Record<string, SeoMeta> = {
  ...
  "/services": {
    ...
    jsonLd: [
      webPage(
        "/services",
        "Services & Pricing | MehyarSoft Software Automation Consulting",
        "MehyarSoft consulting offers for audits, cleanup, missed-call follow-up, automation, integrations, retainers, and custom software.",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
      ...
      faq([
        {
          question: "What consulting services does MehyarSoft offer?",
          answer:
            "MehyarSoft LLC offers practical tech audits, website and booking cleanup, consent-safe missed-call follow-up flows, internal automation sprints, systems integration consulting, support retainers, and custom software builds.",
        },
        {
          question: "What is a local business tech audit?",
          answer:
            "A local business tech audit reviews the public website, booking path, phone and email response, CRM or spreadsheet use, and manual admin bottlenecks to find the highest-value fixes before adding more tools.",
        },
        {
          question: "What is a missed-call follow-up flow?",
          answer:
            "A missed-call follow-up flow turns inbound intent into a lead record, owner notification, and consent-aware SMS or email response with opt-out and suppression planning before scale.",
        },
        {
          question: "When should a business build custom software?",
          answer:
            "Custom software makes sense when the workflow is stable, repeated, important, and constrained by off-the-shelf tools that create more manual work than they remove.",
        },
      ]),
    ],
  },
  ...
  "/portfolio": {
    ...
    jsonLd: [
      webPage(
        "/portfolio",
        "Engagement Patterns | MehyarSoft Consulting Work Examples",
        "Reference engagement patterns for MehyarSoft consulting work.",
        "CollectionPage",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
        { name: "Engagement Patterns", path: "/portfolio" },
      ]),
      {
        "@type": "ItemList",
  ...
  "/blog": {
    ...
    jsonLd: [
      webPage(
        "/blog",
        "Blog & Insights | MehyarSoft Tech Audit & Automation Notes",
        "Practical notes on tech audits, follow-up, automation, and custom software decisions.",
        "Blog",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
  ...
  "/newsletter": {
    ...
    jsonLd: [
      webPage(
        "/newsletter",
        "Free AI Automation Checklist | MehyarSoft",
        "Free checklist for finding lead leaks and safe automation opportunities.",
        "WebPage",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
        { name: "Free Checklist", path: "/newsletter" },
      ]),
      faq([
        {
          question: "What is in the free AI automation checklist?",
          answer:
            "The checklist helps owners review website flow, missed calls, booking friction, follow-up gaps, disconnected systems, and manual work that may be safe to automate.",
        },
        {
          question: "Does joining the checklist require a paid audit?",
          answer:
            "No. The checklist is free. After signup, businesses that want founder-led diagnosis can request the focused $330 audit.",
        },
      ]),
    ],
  },
  ...
  "/about": {
    ...
    jsonLd: [
      webPage(
        "/about",
        "Founder Story | MehyarSoft LLC NYC Software Consultant",
        "Meet Mehyar Swelim, Syrian founder of MehyarSoft LLC in NYC.",
        "AboutPage",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
        { name: "Founder Story", path: "/about" },
      ]),
      organization,
      {
        "@type": "Person",
      ...
      faq([
        {
          ...
          answer:
            "MehyarSoft LLC was founded by Mehyar Swelim, a Syrian founder in New York City and professional software engineer focused on practical software, systems, and automation consulting.",
        },
      ]),
    ],
  },
  "/330": {
    ...
    jsonLd: [
      ...
      breadcrumbs([
        { name: "Home", path: "/" },
        { name: "$330 Audit", path: "/micro-offer" },
      ]),
      {
        "@type": "Service",
  ...
  "/contact": {
    ...
    jsonLd: [
      webPage(
        "/contact",
        "Contact MehyarSoft | Request a Tech Audit or Consulting Call",
        "Contact MehyarSoft LLC to request a practical consulting next step.",
        "ContactPage",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
      ...
      faq([
        {
          ...
          answer:
            "Use the MehyarSoft contact page or email info@mehyar.us with your business type, current workflow problem, tools involved, timeline, and budget range if known. Do not send passwords, API keys, PHI, payment data, or confidential files through public channels.",
        },
      ]),
    ],
  },
  "/booking": {
    ...
    jsonLd: [
      webPage(
        "/booking",
        "Book a Consulting Call | MehyarSoft",
        "Service-specific booking request with safe manual scheduling fallback.",
        "ContactPage",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
      ...
      faq([
        {
          ...
          answer:
            "Use the booking request path to choose a service and preferred time window. MehyarSoft only confirms a calendar event after explicit confirmation; otherwise you will receive available times manually.",
        },
      ]),
    ],
  },
  ...
  "/privacy-policy": {
    ...
    jsonLd: [
      webPage(
        "/privacy-policy",
        "Privacy Policy | MehyarSoft LLC",
        "MehyarSoft privacy policy for contact requests, analytics, follow-up, and sensitive-data cautions.",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
  ...
  "/terms": {
    ...
    jsonLd: [
      webPage(
        "/terms",
        "Terms of Service | MehyarSoft LLC",
        "MehyarSoft website terms covering informational content, consulting engagement boundaries, and sensitive submissions.",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
  ...
  "/sitemap": {
    ...
    jsonLd: [
      webPage(
        "/sitemap",
        "Sitemap | MehyarSoft LLC",
        "Human-readable route directory for MehyarSoft public pages.",
      ),
      breadcrumbs([
        { name: "Home", path: "/" },
...
const ensureElement = <T extends HTMLElement>(
  selector: string,
  create: () => T,
) => {
