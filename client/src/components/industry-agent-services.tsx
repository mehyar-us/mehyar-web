import { ArrowRight, Bot, BrainCircuit, CheckCircle2, Clock3, MessageSquareText, ShieldCheck, Smartphone, Workflow } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
          <div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {["Private, isolated business environment", "Approved channels and authorized users only", "Human approval for sending and risky actions", "Monitoring, backups, maintenance, and support"].map((item) => <div key={item} className="flex gap-2 rounded-2xl border border-border bg-card p-4 text-sm leading-6 text-foreground"><ShieldCheck className="mt-1 h-4 w-4 shrink-0 text-brand-700 dark:text-brand-100" />{item}</div>)}
            </div>
          </div>
        ...
        <div className="mt-14">
          ...
          <div className="mt-7 grid gap-5 lg:grid-cols-3">
            {managedAgentOffers.map((offer) => <article key={offer.name} className="overflow-hidden rounded-[1.5rem] border border-border bg-card shadow-sm"><img src={offer.image} alt="" className="h-44 w-full object-cover" loading="lazy" /><div className="p-5"><h3 className="text-xl font-semibold text-foreground">{offer.name}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{offer.bestFor}</p><div className="mt-5 space-y-2"><p className="rounded-xl bg-brand-950 p-3 text-sm font-semibold text-white">Monthly: {offer.managed}</p><p className="rounded-xl bg-secondary p-3 text-sm font-semibold text-secondary-foreground">Annual: {offer.annual}</p><p className="rounded-xl border border-border p-3 text-sm font-semibold text-foreground">Your infrastructure: {offer.owned}</p><p className="text-xs leading-5 text-muted-foreground">{offer.ownedNote}</p></div><ul className="mt-5 space-y-2 text-sm leading-6 text-muted-foreground">{offer.includes.map((item) => <li key={item} className="flex gap-2"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-brand-700 dark:text-brand-100" />{item}</li>)}</ul><Link href={`/contact?service=systems_consulting&industry=${encodeURIComponent(industry.shortName)}&offer=${encodeURIComponent(offer.name)}`} className={buttonVariants({ variant: "outline", className: "mt-6 w-full" })}>Ask about this setup<ArrowRight className="ml-2 h-4 w-4" /></Link></div></article>)}
          </div>
        </div>
