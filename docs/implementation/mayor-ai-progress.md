...
Latest combined service verification: **90 tests passed across eight files** in local workerd, covering tenant isolation, signed OAuth fixtures, actual unchanged legacy handlers, catalogs, connector adapters, billing and model-call reservations. TypeScript, the 503-file protection guard and independent Worker dry-run bundle passed. This includes regressions for different Stripe events arriving concurrently, setup-plan mismatch, refund ownership conflicts, revoked memberships, cross-business agent binding and failed model attempts. Local `/api/health`, `/api/session`, `/api/auth/capabilities` and `/api/catalog` return 200/no-store; unauthenticated `/api/tenants` returns 401.

Frontend TypeScript/production build and **19 Chromium browser tests passed**, including accessibility checks on sign-in, conversation and billing. Authenticated browser tests use explicit network fixtures. Actual unauthenticated desktop/mobile captures use the running local Worker without interception; both providers correctly report unconfigured. The production PWA shell reloads offline with seven public assets cached and no API/auth/query/private data. These prove layout, contract behavior and shell caching, not live OAuth, mobile installation, payments or automation. Fixture screenshots are explicitly distinguished from the actual local page.
...
| G Roles/authority | Current membership checks, private conversations, owner memory controls | Resource-level staff restrictions, policy broker, approvals, temporary support UX, queued revocation |
...
| U Tests | 90 runtime tests, frontend browser suites and static checks | Entire provider/security/recovery/performance matrix, real mobile and live accounts |
...
2. Implement the deterministic tool-policy/approval/budget broker, then provider token refresh and synchronization. Connect the tested Google/Microsoft adapters and all external effects only through that broker.
