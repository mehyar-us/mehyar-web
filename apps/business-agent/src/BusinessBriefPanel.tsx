...
type IndustryQuestion={field:string;question:string;answer:string;answered:boolean;fromBrief:boolean};
type Result={industryQuestions?:IndustryQuestion[];brief:Brief;identityVerified:false;authorizesActions:false;unresolvedQuestions:{field:string;question:string}[];recommendations:Recommendation[];industryOptions?:{id:string;name:string}[];sources?:Source[]};
...
export const businessDetailLabel=(key:string)=>/^industryAnswers\.[a-z_]+$/.test(key)?'Industry setup':fields.find(([field])=>field===key&&field!=='industryPack')?.[1];
...
function validIndustry(data:Result){const answers=data.brief.industryAnswers;return (answers===undefined||!!answers&&typeof answers==='object'&&!Array.isArray(answers)&&Object.keys(answers).length<=5&&Object.values(answers).every(v=>typeof v==='string'&&v.length<=2000))&&(data.industryQuestions===undefined||Array.isArray(data.industryQuestions)&&data.industryQuestions.length<=5&&data.industryQuestions.every(q=>q&&typeof q.field==='string'&&typeof q.question==='string'&&typeof q.answer==='string'&&typeof q.answered==='boolean'&&typeof q.fromBrief==='boolean'));}
const money=(n:number)=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(n/100);
export default function BusinessBriefPanel({tenantId,online,canEdit,onUnauthorized,conversationDraft,onDismissConversationDraft}:{tenantId:string;online:boolean;canEdit:boolean;onUnauthorized:(cause:unknown)=>void;conversationDraft?:{id:string;text:string;field?:string;industryPack?:string};onDismissConversationDraft?:()=>void}){
  ...
  const [conversationField,setConversationField]=useState('services'),[conversationText,setConversationText]=useState('');
  useEffect(()=>{setConversationText(conversationDraft?.text??'');setConversationField(conversationDraft?.field&&businessDetailLabel(conversationDraft.field)?conversationDraft.field:'services');},[conversationDraft]);
  const load=useCallback(async()=>{
    active.current?.abort();const c=new AbortController();active.current=c;setResult(null);setError('');setBusy(true);
    try{const data=await api<Result>(`/api/tenants/${encodeURIComponent(tenantId)}/business-brief`,{signal:c.signal});if(!valid(data)||!validSources(data.sources)||!validIndustry(data)||(data.industryOptions!==undefined&&(!Array.isArray(data.industryOptions)||data.industryOptions.length>10||!data.industryOptions.every(option=>typeof option.id==='string'&&typeof option.name==='string'))))throw new Error('The business brief returned an unexpected response. Please reload.');if(!c.signal.aborted){setResult(data);setDraft(data.brief.fields);setIndustryAnswers(data.brief.industryAnswers??{});setSourceIds(Object.fromEntries(Object.entries(data.brief.sources??{}).map(([key,source])=>[key,source.id])));setReviewed(false);}}
  ...
  async function save(){
    ...
    const request=pending.current??{key:crypto.randomUUID(),revision:result.brief.revision,body:JSON.stringify({expectedRevision:result.brief.revision,reviewed:true,fields:Object.fromEntries(fields.map(([key])=>[key,draft[key].trim()])),sourceIds,industryAnswers})};pending.current=request;
    ...
    }catch(cause){if(!c.signal.aborted){if(cause instanceof ApiError&&['brief_revision_conflict','brief_source_changed','invalid_industry_answers'].includes(cause.code)){pending.current=null;setReviewed(false);}setError(cause instanceof Error?cause.message:'The brief could not be saved. Retry the same update.');if(cause instanceof ApiError&&cause.status===401)onUnauthorized(cause);}}
    finally{if(!c.signal.aborted)setBusy(false);}
  }
  function field(key:string,label:string){
    if(key==='industryPack')return <div className="field" key={key}><label htmlFor={`${tenantId}-brief-industry`}>{label}</label><select id={`${tenantId}-brief-industry`} value={draft.industryPack??''} disabled={!canEdit||busy||!!pending.current} onChange={event=>{setDraft(current=>({...current,industryPack:event.target.value}));setIndustryAnswers({});setReviewed(false);}}><option value="">General recommendations</option>{result?.industryOptions?.map(option=><option key={option.id} value={option.id}>{option.name}</option>)}</select><p className="small muted">Choose the closest fit. This changes suggestions only and does not activate workflows.</p></div>;const source=result?.brief.sources?.[key];return <div className="field" key={key}><label htmlFor={`${tenantId}-brief-${key}`}>{label}</label><textarea id={`${tenantId}-brief-${key}`} rows={2} maxLength={2000} value={draft[key]??''} disabled={!canEdit||busy||!!pending.current} onChange={event=>{setDraft(current=>({...current,[key]:event.target.value}));setSourceIds(current=>{const next={...current};delete next[key];return next;});setReviewed(false);}}/>{source&&sourceIds[key]===source.id&&<a href={source.sourceUrl} target="_blank" rel="noopener noreferrer">Saved source for {label.toLowerCase()}</a>}</div>;}
  function industryField(q:IndustryQuestion){return <div className="field" key={q.field}><label htmlFor={`${tenantId}-industry-${q.field}`}>{q.question}</label><textarea id={`${tenantId}-industry-${q.field}`} rows={2} maxLength={2000} value={industryAnswers[q.field]??''} disabled={!canEdit||busy||!!pending.current} onChange={event=>{setIndustryAnswers(current=>({...current,[q.field]:event.target.value}));setReviewed(false);}}/></div>;}
  const industryTarget=conversationField.startsWith('industryAnswers.')?conversationField.slice('industryAnswers.'.length):null;
  const industryQuestion=result?.industryQuestions?.find(q=>q.field===industryTarget);
  const staleIndustrySuggestion=!!conversationDraft?.field?.startsWith('industryAnswers.')&&(!conversationDraft.industryPack||conversationDraft.industryPack!==draft.industryPack||conversationDraft.industryPack!==result?.brief.fields.industryPack);
  const invalidTarget=staleIndustrySuggestion||!!industryTarget&&(!industryQuestion||draft.industryPack!==result?.brief.fields.industryPack);
  const currentConversationValue=industryTarget?(industryAnswers[industryTarget]??industryQuestion?.answer??''):draft[conversationField];
  function copyConversation(){
    if(invalidTarget)return;
    if(industryTarget)setIndustryAnswers(current=>({...current,[industryTarget]:conversationText.trim()}));
    else{setDraft(current=>({...current,[conversationField]:conversationText.trim()}));setSourceIds(current=>{const next={...current};delete next[conversationField];return next;});}
    setReviewed(false);setNotice('Conversation wording copied to your draft. Review and save to keep it.');onDismissConversationDraft?.();
    const input=document.getElementById(industryTarget?`${tenantId}-industry-${industryTarget}`:`${tenantId}-brief-${conversationField}`);const details=input?.closest('details');if(details)details.open=true;input?.focus();
  }
  return <section className="panel research-panel" aria-label="Business brief" style={{overflowWrap:'anywhere'}}>
    ...
    {!online?<p>Reconnect to view or update the business brief.</p>:!result?<p>{busy?'Loading business brief…':'Reload the brief to try again.'}</p>:<>
      ...
      {canEdit&&conversationDraft&&<section className="stack" aria-label="Conversation brief draft">
        <h3>From your conversation</h3><p>Choose where this belongs and edit the wording. Copying changes only the draft below; review it before saving.</p>
        {staleIndustrySuggestion&&<p role="alert">Your industry selection changed. Discard this suggestion and ask for an updated one.</p>}
        <label htmlFor={`${tenantId}-conversation-field`}>Business detail to update</label><select id={`${tenantId}-conversation-field`} value={conversationField} disabled={busy||!!pending.current||staleIndustrySuggestion} onChange={event=>setConversationField(event.target.value)}>{fields.filter(([key])=>key!=='industryPack').map(([key,label])=><option key={key} value={key}>{label}</option>)}{result.industryQuestions?.map(q=><option key={q.field} value={`industryAnswers.${q.field}`}>{q.question}</option>)}</select>
        <p>Current draft value: {currentConversationValue||'Not entered'}</p>
        <label htmlFor={`${tenantId}-conversation-text`}>Proposed wording</label><textarea id={`${tenantId}-conversation-text`} value={conversationText} rows={3} maxLength={2000} disabled={busy||!!pending.current} onChange={event=>setConversationText(event.target.value)}/>
        {conversationText.length>2000&&<p>Shorten this answer to 2,000 characters before copying.</p>}
        <button className="button secondary" disabled={busy||!!pending.current||invalidTarget||!conversationText.trim()||conversationText.length>2000} onClick={copyConversation}>{currentConversationValue?'Replace this draft value':'Copy to draft'}</button>
        <button className="button secondary" disabled={busy||!!pending.current} onClick={onDismissConversationDraft}>Discard conversation draft</button>
      </section>}
      <form className="stack" onSubmit={event=>{event.preventDefault();void save();}}>
        ...
        {draft.industryPack!==result.brief.fields.industryPack?<p>Save your reviewed industry choice to see its setup questions. Previous industry answers will be cleared.</p>:!!result.industryQuestions?.length&&<div className="stack">
          {result.industryQuestions.some(q=>!q.answered)&&<><h3>Remaining industry setup</h3>{result.industryQuestions.filter(q=>!q.answered).map(industryField)}</>}
          {result.industryQuestions.some(q=>q.answered)&&<details><summary>Saved industry answers</summary><div className="stack">{result.industryQuestions.filter(q=>q.answered).map(q=>q.fromBrief&&!Object.hasOwn(industryAnswers,q.field)?<p key={q.field}>{q.question} {q.answer} (From your business details.)</p>:industryField(q))}</div></details>}
        </div>}
