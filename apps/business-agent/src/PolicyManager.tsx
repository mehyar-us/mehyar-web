...
export default function PolicyManager({tenantId,online,onUnauthorized,onChanged}:{tenantId:string;online:boolean;onUnauthorized:(cause:unknown)=>void;onChanged:()=>void}) {
  ...
  async function save(event:FormEvent) {
    ...
    const {id,version,name,trigger,operation,provider,grantId,mode,resources,recipients,template,startsAt,expiresAt,maxActionsPerDay,maxCostMicrosPerDay,escalation,enabled}=draft;
  ...
  return <section className="panel calendar-policy" aria-label="Saved policies">
    ...
    {!draft&&policies.map(policy=><article key={policy.id} aria-label={`Policy: ${policy.name}`}>
      ...
      <button className="button secondary" disabled={!online||busy||loading} onClick={()=>{setDraft({...policy});setError('');setNotice('');}}>Edit {policy.name}</button>
    </article>)}
    {draft&&<form onSubmit={event=>void save(event)} aria-label="Edit policy">
      <fieldset disabled={!online||busy} style={{border:0,padding:0,minWidth:0}}>
        ...
        <p>Calendar or message resources: {draft.resources.join(', ')}</p><p>Allowed recipients: {draft.recipients.join(', ')}</p>
        <p>Provider: {draft.provider} · Review mode: {draft.mode} · Expires {new Date(draft.expiresAt).toLocaleString()}</p>
        ...
        <p>Account, resources, recipients, review mode, spending limit and expiry stay as shown in the saved policy. Create a new policy to change those permissions.</p>
