  ...
  return <section className="panel" aria-label="Manage team">
    ...
    {!online?<p>Reconnect to manage your team.</p>:<>
      ...
      {data&&<>
        ...
        <h3>Invitations</h3>{!data.invitations.length?<p>No invitations yet.</p>:<ul>{data.invitations.map(invitation=><li key={invitation.id}><strong>{invitation.email}</strong><p>{invitation.role} · {invitation.status} · expires {new Date(invitation.expiresAt).toLocaleString()}</p><InvitationEmail email={invitation.email} delivery={invitation.emailDelivery} enabled={data.emailQueueEnabled===true} pending={invitation.status==='pending'} disabled={busy||!online} onCancel={()=>void(async()=>{if(await mutate(`invitations/${invitation.id}/email/cancel`,{})&&live.current)await refresh();})()} onQueue={()=>void(async()=>{if(await mutate(`invitations/${invitation.id}/email`,{})&&live.current)await refresh();})()}/>{invitation.status==='pending'&&<button className="button secondary" disabled={busy} onClick={()=>void(async()=>{if(await mutate(`invitations/${invitation.id}/revoke`,{})&&live.current)await refresh();})()}>Revoke invitation for {invitation.email}</button>}</li>)}</ul>}
