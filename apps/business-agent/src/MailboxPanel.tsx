...
type Status={state:keyof typeof labels;setupEnabled:boolean;pending:number;lastObservedAt:string|null};
...
export default function MailboxPanel({tenantId,grantId,online,onUnauthorized}:{tenantId:string;grantId:string;online:boolean;onUnauthorized:(error:unknown)=>void}) {
  ...
  const load=useCallback(async(start=false)=>{
    ...
    try {
      const result=await api<unknown>(`/api/tenants/${encodeURIComponent(tenantId)}/connections/${encodeURIComponent(grantId)}/mailbox`,
        {signal:controller.signal,...start?{method:'POST',body:'{}'}:{}});
      if(!valid(result))throw new Error('Mailbox status could not be verified.');
  ...
  },[tenantId,grantId,online,onUnauthorized]);
  ...
  return <div aria-label="Mailbox monitoring" className="notice mailbox-panel"><div>
    ...
    {!online?<p>Reconnect to check mailbox status.</p>:busy?<p role="status">Checking mailbox…</p>:error?<p role="alert">{error} Refresh status before trying setup again.</p>:status?<>
      ...
      {status.state==='not_started'&&!status.setupEnabled&&<p>Setup becomes available after your subscription and mailbox service are activated.</p>}
