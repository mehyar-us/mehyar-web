...
type Directory={items:Item[];withheld:number;nextCursor?:string};
...
function valid(value:unknown):value is Directory{
  ...
  return !!page&&Number.isSafeInteger(page.withheld)&&page.withheld>=0&&page.withheld<=10
...
export default function MailboxAnalyses({tenantId,grantId,onUnauthorized}:{tenantId:string;grantId:string;onUnauthorized:(error:unknown)=>void}){
  ...
  useEffect(()=>{setItems([]);setCursor(undefined);setWithheld(0);setLoaded(false);setError('');setBusy(false);seen.current.clear();return()=>request.current?.abort();},[tenantId,grantId]);
  async function load(more=false){
    ...
    try{
      ...
      setItems(old=>more?[...old,...result.items]:result.items);setWithheld(old=>more?old+result.withheld:result.withheld);setCursor(result.nextCursor);setLoaded(true);
